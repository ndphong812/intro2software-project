...
const schema = yup.object().shape({
  ...
  detail: yup.string().required("Chi tiết sản phẩm không được để trống"),
  stock: yup.string().required("Số lượng tồn kho không được để trống"),
  original_price: yup.string().required("Giá không được để trống"),
  sale_price: yup.string(),
  type: yup.string().required("Type phải là điện thoại/tablet/laptop"),
...
const NewEditProduct: React.FC<Props> = ({
  product,
  isOpen,
  handleOpen,
  handleClose,
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (type == "new") {
      ...
      const response = await dispatch(addProductSeller(formData));
      SwalAlert(
        "Thành công",
        "Tạo sản phẩm thành công, chờ admin duyệt",
        "success"
      );
    } else {
      const editData: any = {
  ...
  return (
    <div style={{ marginBottom: "40px" }}>
      <div
        className="manage-product-list-btns-child"
      ...
      <Dialog open={isOpen} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>
            Sau khi ấn Hoàn tất, sản phẩm của bạn sẽ được gửi đến cho Admin, và
            chờ duyệt.
          </DialogContentText>
          <form onSubmit={onSubmit} className="new-product-content">
            ...
            />
            <ImageUploader
              defaultImage={product.image_link}
