...
class UserController {
  static listAll = async (req: Request, res: Response) => {
    ...
    const users = await userRepository.find({
      select: ["user_id", "email", "role", "fullname"], // don't send password in response
    });
  ...
  static getOneById = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(401)
        .json({ status: "failure", message: "Người dùng không tồn tại." });
    }
  };
  ...
  static newUser = async (req: Request, res: Response) => {
    ...
    if (!user.email.includes("@")) {
      return res
        .status(409)
        .json({ status: "failure", message: "Định dạng email không đúng." });
    }
    ...
    if (user.hashpass.length < 6) {
      return res
        .status(409)
        .json({ status: "failure", message: "Password phải ít nhất 6 kí tự." });
    }
    ...
    if (user.role === "admin") {
      return res
        ...
        .json({
          status: "failure",
          message: "Bạn không thể thiết lập 1 admin mới.",
        });
    }
    ...
    try {
      ...
      return res
        .status(409)
        .json({ status: "failure", message: "Email này đã được sử dụng." });
    }

    //If all ok, send 201 response
    return res
      .status(201)
      .json({ status: "success", message: "Đã tạo tài khoản thành công." });
  };
  ...
  static editUser = async (req: Request, res: Response) => {
    ...
    if (!user_id) {
      return res
        ...
        .json({
          status: "failure",
          message: "Thông tin người dùng cần cập nhật không chính xác.",
        });
    }
    ...
    if (role === "admin") {
      return res
        .status(401)
        .json({ status: "failure", message: "Không thể tạo ra 1 admin mới." });
    }
    ...
    try {
      ...
      return res
        .status(404)
        .json({ status: "failure", message: "Không tìm thấy người dùng này." });
    }
    ...
    if (user.role === "admin") {
      return res
        ...
        .json({
          status: "failure",
          message: "Không thể chỉnh sửa thông tin người dùng này.",
        });
    }
    ...
    try {
      await userRepository.update({ user_id: user_id }, user_update);

      return res
        .status(201)
        .json({ status: "sucess", message: "Đã cập nhật thành công." });
    } catch (error) {
      return res
        .status(409)
        .json({ status: "failure", message: "Email này đã được sủ dụng rồi." });
    }
    //success
  };
  ...
  static deleteUser = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ status: "success", message: "Xóa người dùng thành công." });
    } catch (error) {
      return res
        .status(404)
        .json({ status: "failure", message: "Người dùng này không tồn tại." });
    }
  };
  ...
  static updateProfile = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        ...
        .json({
          status: "failure",
          message: "Cập nhật hồ sơ cá nhân thất bại.",
        });
    }
  };
}
